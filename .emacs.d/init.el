...
(setq ls-lisp-use-insert-directory-program t)  ;; 不走 elisp 模拟
(setq dired-use-ls-dired t)
(add-hook 'dired-mode-hook
          (lambda ()
            (when (string-prefix-p
                   (file-truename (expand-file-name "~/Downloads/"))
                   (file-truename (expand-file-name default-directory)))
              (setq-local dired-listing-switches "-alht --time-style=long-iso"))))
